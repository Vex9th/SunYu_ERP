...
发布基线为 `a5ed7c076acbd5ab78aa224c9286dc3d60274bbd`。已经可用的能力：
...
截至 2026-08-29，后端已在上述基线上连续执行 `005` 至 `009`，并挂载项目阶段、采购、库存和人员施工第一批路由。当前已形成的真实闭环是：固定 18 阶段、采购模板与手工清单、采购单、到货入库、库存调整、项目领用、施工员、项目排单和当日上工批量提交。

这不代表全部业务已经完成。文件上传 API、报价/合同/三段收款 API、Excel 上传预检与确认、供应商付款/发票、隐藏成本报价单、施工日报与垫资、调试变更、验收质保、发票售后和完整经营仪表台仍按后续任务实施。`006` 与 `009` 中提前建立的连续表只是数据底座，不得在前端伪装成可用功能。
